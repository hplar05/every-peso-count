...
import { StaffQRPanel } from "./staff-qr"
...
export default async function StaffAccountsPage() {
  ...
  const { data: currentUser } = await supabase.from("officials").select("role").eq("id", user.id).single()
                ...
                <tr key={official.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                      official.status === "approved" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                    }`}>
                  ...
                  <td className="px-6 py-4 text-center">
                    {official.status === "approved" && <StaffQRPanel official={official} />}
                  </td>
                  <td className="px-6 py-4 text-right">
                    {official.role !== "admin" && official.status === "approved" && (
                      <form action={async () => {
                        ...
                        <button type="submit" className="inline-flex items-center gap-1 text-red-600 hover:text-red-800 transition-colors text-sm">
